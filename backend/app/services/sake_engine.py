class SakeEngine:
    def build_filters(self, filter_dict: dict):
        """Clean up null filters for ChromaDB metadata filtering."""
        # Only passing 'Hard Specs' that actually have values
        # Example: {'prefecture': 'Gifu', 'polishing_ratio': None} -> {'prefecture': 'Gifu'}
    ...
    def format_for_llm(self, suggestion_results, knowledge_results):
        ...
        if suggestion_results and suggestion_results.get('documents'):
        ...
        if knowledge_results and knowledge_results.get('documents'):
